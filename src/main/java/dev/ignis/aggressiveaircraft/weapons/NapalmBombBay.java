...
import static dev.ignis.aggressiveaircraft.entities.ModEntities.NAPALM_BOMB;

public class NapalmBombBay extends BulletWeapon {
    private float cooldown = 0.0f;

    public NapalmBombBay(VehicleEntity entity, ItemStack stack, WeaponMount mount, int slot) {
    ...
    @Override
    public void tick() {
        float cooldownSeconds = (float)(double) ModConfig.NAPALM_BOMB_COOLDOWN.get();
        cooldown -= 1.0f / 20.0f;
    ...
    public void fire(Vector3f direction) {
        String ammoId = ModConfig.NAPALM_BOMB_AMMO.get();
        int consumption = ModConfig.NAPALM_BOMB_AMMO_CONSUMPTION.get();
